...
## 长途               ← 前置块 ×3：大段事务的细节住在这里，事件用 detail: 引用
```trip-transports```
...
destination: Seattle, WA, USA # 必需 · 城市, 地区, 国家 —— 首页卡片展示用
...
大段事务的**细节全部前置声明**，每日行程里的事件用 `detail: 名字` 一句话引用 ——
事件专注「发生了什么」，三个视图从前置块取同一份数：列表要信息模块、
日历要区间带与出发时刻、预算要金额。

三个块共用同一套规则：

- `what` 必需，是事件引用的名字 —— **三个块之间全文唯一**，重名报错
- `cost` 是这笔预订的钱（多人加总），进预算统计：长途记到首次引用那天、
  住宿记到入住日、租车记到取车日。事件自己的 `cost` 只写当场散项（门票、停车、加油），不要两头写
- **首次引用**（按日期序）的卡片长出信息模块 / 票面时间轴；之后再引用
  （回酒店、还车）是简单提及 —— 普通卡片，住/行筛选视图里自动去重
- 引用不存在的名字报错（带「是否指…」建议）；没被任何事件引用的记录发警告
- 除必需字段外**不知道的直接省略、不要编** —— UI 留「待填」空位，之后人工或 agent 补

---

## ` ```trip-transports `（长途，可选）

去程航班、新干线、跨城巴士、轮渡这类**带票面明细的跨城移动**。
细节（承运方、班次、时刻、票价、行李、退改、中转）全部写在这里：

```yaml
- what: 去程 · 汇合                # 必需：这段长途叫什么，事件按它引用，全文唯一
  cost: $842 + 她的票待定           # 总预算（多人加总），进预算统计；票面单价写各条的 price
  transport:                       # 单条写一个 map，多人写列表（一人一条）；紧凑的 {…} 写法同样有效
    - traveler: 她                  # 谁的（多人汇合时区分）
      mode: flight                 # flight/rail/hsr/drive/ferry/bus…，默认 flight，决定图标与文案
      carrier: 达美 Delta           # 航司/铁路公司/船司
      number: DL281                # 航班号/车次/船班
      from: PVG T1                 # 出发点
      to: SEA                      # 到达点
      dep_time: "10:15"            # 出发当地时间
      ...
      stops:                       # 中转/停靠，按顺序
        - airport: ICN             # 到达点；异地换乘的再出发点写 dep_airport（同地不写）
          arr_time: "13:55"        # 到达当地时间
...
time: "13:11"
category: flight
detail: 去程 · 汇合               # 引用；解析时明细灌进首次引用的事件，票面时间轴照常渲染
```

UI 渲染成机票式时间轴：左出发、右到达、实线行进、虚线中转/停留，段长按时长分配。
**不限飞机** —— `mode` 决定图标与文案，火车/自驾/轮渡/长途大巴同样适用。

- 除 `what` 外都可空。票常常晚于行程定下来，UI 会画完整骨架、缺的字段留「待填」空位（`category: flight` 的事件没写记录也画空骨架）。
- 多目的地行程中间的长途移动（如西雅图 → 波特兰的火车）同样是一条记录 + 一个引用它的事件。
- `duration` / `wait` / `leg` 接受 `11h55m`、`2h`、`45m`、`11 小时 55 分`、`"11:55"`、纯数字（分钟）。
- `leg` 不写则按「全程 − Σ停留 − Σ已填段」均分，各段与停留之和恒等于全程。
- 事件本身的 `time` 仍按 TripMD 通用规则写（抵达事件写到达时刻、出发事件写出发时刻），记录里的时间是给票面时间轴用的。
...
推导规则：出发日默认取首次引用它的事件所在那天；后一个时刻比前一个小就进一天；
...
入住的**动作**照常写成当天的 `hotel` / `homestay` 事件并挂 `detail: 酒店名` ——
通常入住卡就是首次引用，住宿模块长在它身上。
...
取/还的**动作**照常写成当天的 `drive` 事件并挂 `detail: 车名` ——
首次引用的取车卡长出租期模块，还车卡自动去重。
...
> 住在哪**不写在天里** —— 写成顶层 ` ```trip-stays ` 的一条区间记录（见上）。
...
| `detail` | | **前置记录引用**：写顶层 `trip-transports` / `trip-stays` / `trip-rentals` 里某条记录的 `what`。首次引用（按日期序）的卡片长出信息模块/票面时间轴，之后的引用是简单提及（住/行视图里去重隐藏）。引用不存在或跨块重名都会报错。事件上**不再**写 `transport:` 内联明细 |
...
大段事务（长途 / 住宿 / 长租）的细节不写在事件上，事件只留一根指针：

```yaml
### 抵达西雅图 · 汇合
time: "13:11"
category: flight
detail: 去程 · 汇合        # 首次引用的卡片长出票面时间轴 / 信息模块
```

首次引用（按日期序）显示完整信息，之后再引用（回酒店、还车）是简单提及 ——
普通卡片，住/行筛选视图里自动去重。三个前置块的写法见上方各节；
事件上写 `transport:` 内联明细会报迁移错误。
